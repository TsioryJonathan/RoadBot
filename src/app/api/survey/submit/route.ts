...
export async function POST(request: Request) {
  ...
  try {
    /*const response = await Gemini.models.generateContent({
      ...
      contents:
        "These are the questions and current situation of the user: " + payload,
      config: {
      ...
      messages: [
        {
          role: "user",
          content:
            "These are the questions and current situation of the user: " +
            payload,
        },
        {
          role: "system",
          content: `You are a career advisor. Based on the user's answers to the survey, suggest the top 3 most suitable career paths for them. Provide a brief explanation for each suggestion.The structure of your answer should only be a JSON without the indication that it is a json just the format below and dont send the backticks and should not contain any other text and should follow this format: {
        "jobs": [
            {
                "id": just generate an uuid,
                "title": "Title of the career path",
